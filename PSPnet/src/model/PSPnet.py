...
class PSPNet(nn.Cell):
    def __init__(
        ...
        if backbone == "resnet50":
            self.backbone = ResNet(pretrained=pretrained,pretrained_path=pretrained_path)
            aux_channel = 1024
            out_channel = 2048
        else:
            raise ValueError(
        ...
        if self.aux_branch:
            ...
            self.auxiliary_branch = nn.SequentialCell(
                nn.Conv2d(
                    aux_channel, out_channel // 8, kernel_size=3, padding=1, has_bias=False,pad_mode="pad"
                ),
        ...
        for model in models:
            for _, cell in model.cells_and_names():
                ...
                if isinstance(cell, nn.Dense):
                    cell.weight.set_data(weight_init.initializer(weight_init.TruncatedNormal(0.01),
                                                                 cell.weight.shape,
                                                                 cell.weight.dtype))
